...
let cms = {
    ...
    'desktop': [
        ...
        `<a onmousedown="window.open('https://github.com/babiwawa/win12','_blank');" win12_title="https://github.com/babiwawa/win12" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)"><i class="bi bi-github"></i> 在 Github 中查看此项目</a>`,
...
let nts = {
    ...
    'feedback': {
        cnt: `<p class="tit">反馈</p>
            <p>我们非常注重用户的体验与反馈</p>
            <list class="new">
                <a class="a" onclick="window.open('https://github.com/babiwawa/win12/issues','_blank');" win12_title="在浏览器新窗口打开链接" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)">在github上提交issue(需要github账户，会得到更高重视)</a>
...
let apps = {
    setting: {
        ...
        theme_get: () => {
            ...
            // 实时获取主题
            $.get('https://api.github.com/repos/babiwawa/win12-theme/contents').then(cs => {
                cs.forEach(c => {
                    if (c.type == 'dir') {
                        $.get(c.url).then(cnt => {
                            cnt.forEach(cn => {
                                if (cn.name == 'theme.json') {
                                    $.getJSON('https://babiwawa.github.io/win12-theme/' + cn.path).then(inf => {
                                        infjs = inf;
                                        if ($('#set-theme>loading').length)
                                            $('#set-theme').html('');
                                        $('#set-theme').append(`<a class="a act" onclick="apps.setting.theme_set('${c.name}')" style="background-image:url('https://babiwawa.github.io/win12-theme/${c.name}/view.jpg')">${c.name}</a>`);
                                    })
                                }
                            })
                        })
        ...
        theme_set: (infp) => {
            $.get('https://api.github.com/repos/babiwawa/win12-theme/contents/' + infp).then(cnt => {
                console.log('https://api.github.com/repos/babiwawa/win12-theme/contents/' + infp);
                cnt.forEach(cn => {
                    if (cn.name == 'theme.json') {
                        $.getJSON('https://babiwawa.github.io/win12-theme/' + cn.path).then(inf => {
                            infjs = inf;
                            cnt.forEach(fbg => {
                                ...
                                if (fbg.name == infjs.bg) {
                                    $(':root').css('--bgul', `url('https://babiwawa.github.io/win12-theme/${fbg.path}')`);
                                    $(':root').css('--theme-1', infjs.color1);
                                    $(':root').css('--theme-2', infjs.color2);
                                    $(':root').css('--href', infjs.href);
                                    // $('#set-theme').append(`<a class="a act" onclick="apps.setting.theme_set(\`(${inf})\`)" style="background-image:url('https://babiwawa.github.io/win12-theme/${fbg.path}')">${c.name}</a>`);
                                }
                            })
                        })
                    }
    ...
    about: {
        ...
        get: () => {
            ...
            <circle cx="8px" cy="8px" r="7px" style="stroke:#2983cc;stroke-width:3px;"></circle></svg></loading>`)
            // 实时获取项目贡献者
            $.get('https://api.github.com/repos/babiwawa/win12/contributors').then(cs => {
                setTimeout(() => {
                    $('#contri').html('');
                    cs.forEach(c => {
...
let copilot={
    ...
    init:()=>{
        ...
        1.Windows 12 网页版是一个开源项目，由爸比娃娃原创, 使用 Html,css,js，在网络上模拟、创新操作系统
        2.项目的地址是github.com/babiwawa/win12
        3.此项目使用EPL2.0开源许可
